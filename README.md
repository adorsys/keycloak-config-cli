...
The config files based on the keycloak export files. You can use them to re-import your settings. Keep your files as small as possible. Remove all
UUIDs and all stuff which is default set by keycloak.

[moped.json](./contrib/example-config/moped.json) is a full working example file you can consider. Other examples located in
the [test resources](./src/test/resources/import-files).
...
We are using [TestContainers](https://www.testcontainers.org/) in our integration tests. The integration tests requires a configured docker
environment.
...
| --import.state                                        | `IMPORT_STATE`                                       | Enable state management. Purge only resources managed by keycloak-config-cli. S.   | `true`      | [MANAGED.md](docs/MANAGED.md)                                                                                                   |
...
| --import.show-diff=true                               | `IMPORT_SHOWDIFF`                                    | Dump diff between import realm config and keycloak realm config (excluding users) | `false`     |                                                                                                                                 |
