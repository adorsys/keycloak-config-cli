...
| --keycloak.authorization              | `KEYCLOAK_AUTHORIZATION`             | pre-acquired bearer token (raw, without `Bearer ` prefix). Bypasses grant-type login when set. No refresh is attempted — the token must outlive the run. | -           |                                                                                                  |
